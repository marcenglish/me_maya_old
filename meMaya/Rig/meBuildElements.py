...
#        else:
#            print obj.name+' has no start joint '+ startJoint
...
def INV_foot(obj):
    ...
    m.parentConstraint(obj.PIKJ[0],obj.name+id+'_plug_start_OUT')
    ...
    #for the foot, inplug should be driven by the IK control, not the resulting end joint transformation    
    m.parentConstraint(obj.connectedToObj.name+'_'+obj.coreJoints[0]+id+'_CTRL',obj.inPlug,mo=True)

    
    ##post build modifactions to parent rig (inverse functionality):

    #output should drive the IK handle, the lengthRatio and the twist_end locator

    m.delete(obj.connectedToObj.name+'_'+obj.coreJoints[0]+'_ikHandle'+id+'_parentConstraint1')
    m.parentConstraint(obj.outPlug,obj.connectedToObj.name+'_'+obj.coreJoints[0]+'_ikHandle'+id,mo=True)

    m.delete(obj.connectedToObj.name+id+'_lenRatio_end_parentConstraint1')
    m.parentConstraint(obj.outPlug,obj.connectedToObj.name+id+'_lenRatio_end')

    m.delete(obj.connectedToObj.name+'_'+obj.connectedToObj.coreJoints[1]+id+'_bendyTwist_end_parentConstraint1')
    m.parentConstraint(obj.outPlug,obj.connectedToObj.name+'_'+obj.connectedToObj.coreJoints[1]+id+'_bendyTwist_end')
    ...
    #fk skeleton should now be driven by the parent rig's fk skeleton
    # m.delete(obj.name+id+'_plug_start_IN_to_'+obj.name+'_FK'+id+'_GRP_PCON')
    #m.parentConstraint(obj.connectedToObj.name+'_FK_'+obj.connectedToObj.coreJoints[-1]+id,obj.name+'_FK'+id+'_GRP')
